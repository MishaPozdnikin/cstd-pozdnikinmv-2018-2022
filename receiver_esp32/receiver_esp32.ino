...
void setup()
{
  ...
  pinMode(bleSelectPin, INPUT_PULLUP);
  pinMode(wifiSelectPin, INPUT_PULLUP);
  modeBle = !digitalRead(3);
  modeWifi = !analogRead(A7);
  
  if (modeBle && !modeWifi) 
  {
    btle.begin("");
  } 
  else if (!modeBle && modeWifi)
  {
    radio.begin();
    radio.setAutoAck(1);
...
void BLE_part() 
{
  ...
  if (btle.listen()) 
  {
    ...
    {
      pinMode(string.charAt(2), OUTPUT);
